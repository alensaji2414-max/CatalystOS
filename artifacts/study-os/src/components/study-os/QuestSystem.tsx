...
import { useStudyStore, Quest, Subject } from '@/lib/study-store';
...
interface QuestCardProps {
  quest: Quest;
  index: number;
  boosts: { xpMultiplier: number; coinMultiplier: number };
  claimingId: string | null;
  subjects: Subject[];
  onClaimReward: (quest: Quest) => void;
}

function QuestCard({ quest, boosts, claimingId, subjects, onClaimReward }: QuestCardProps) {
          ...
          {filteredQuests.length > 0 ? (
            ...
            >
              {filteredQuests.map((quest, index) => (
                <QuestCard key={quest.id} quest={quest} index={index} boosts={boosts} claimingId={claimingId} subjects={subjects} onClaimReward={handleClaimReward} />
              ))}
            </motion.div>
          ) : (
            <motion.div
              ...
              {bossQuests.length > 0 ? (
                ...
                >
                  {bossQuests.map((boss, index) => (
                    <QuestCard key={boss.id} quest={boss} index={index} boosts={boosts} claimingId={claimingId} subjects={subjects} onClaimReward={handleClaimReward} />
                  ))}
                </motion.div>
              ) : (
